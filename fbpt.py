...
def calculate_file_hash(filepath, hash_algorithm=HashAlgorithm.MD5.value):
...
def compare_file_permission(real_mode, least_mode):
    real_u, real_g, real_o = real_mode
    least_u, least_g, least_o = least_mode
    if real_u < least_u or real_g < least_g or real_o < least_o:
        return False
    return True
...
def word_to_pdf(source_file, target_file, word_app):
    try:
    ...
    try:
        word = wc.Dispatch(WORD_APPLICATIONS[word_app])
...
def batch_replace_files(rootdir, pattern, replace):
    check_dir(rootdir)
    for dirpath, _, filenames in os.walk(rootdir):
        for filename in filenames:
            file_path = os.path.join(dirpath, filename)
            tmp_file_path = os.path.join(dirpath, f'{filename}.tmp')
            num_chunk = get_chunk_number(file_path, DEFAULT_CHUNK_SIZE)
            with open(tmp_file_path, 'w+') as outfile:
                with open(file_path, 'r') as infile:
                    for i in range(num_chunk):
                        chunk_data = infile.read(DEFAULT_CHUNK_SIZE)
                        replaced_chunk_data = re.sub(pattern, replace, chunk_data)
                        outfile.write(replaced_chunk_data)
            os.rename(tmp_file_path, file_path)


def batch_file_name_check(rootdir, pattern):
    check_dir(rootdir)
    all_match = True
    for dirpath, _, filenames in os.walk(rootdir):
        for filename in filenames:
...
def batch_file_integrity_check(rootdir, input_hash_file,
    ...
    for file_path in file_paths:
        ...
        real_file_hash = calculate_file_hash(file_path, hash_algorithm=hash_algorithm)

        if expected_file_hash != real_file_hash:
            print(f'mismatch: {file_path} (expected: {expected_file_hash}, real: {real_file_hash})')


def batch_file_permission_check(rootdir, least_mode):
    if len(least_mode) > 3 or any(m < '0' or m > '7' for m in least_mode):
        print_error(f'Error: invalid mode \'{least_mode}\'')
        exit(1)

    for dirpath, dirnames, filenames in os.walk(rootdir):
        for dirname in dirnames:
            pathname = os.path.join(dirpath, dirname)
            real_mode = oct(os.lstat(pathname).st_mode)[-3:]
            if not compare_file_permission(real_mode, least_mode):
                print_error(f'Permission mismatch (path: {pathname}, mode: {real_mode})')
    ...
    for hash_value, files in hashes.items():
        if len(files) > 1:
            ...
            for dup in duplicates:
                ...
                print(f'{action}: {dup}')
    if action == DeduplicateAction.DELETE.value:
        print(f'Free up space: {total_saved / 1024 / 1024:.2f} MiB')
...
def batch_hash_internal(rootdir, hash_values,
    ...
    for filename in os.listdir(rootdir):
        ...
        if os.path.isdir(pathname):
            ...
            hash_values[pathname] = calculate_file_hash(pathname, hash_algorithm=hash_algorithm)
...
def batch_convert_files(src_dir, dest_dir, conversion_type, format='PNG', recursive=False, word_app='MS'):
    check_src_and_make_dest(src_dir, dest_dir)
        ...
        for file in files:
            ...
            if file_ext in config['extensions']:
                ...
                if conversion_type == 'image':
                    ...
                    word_to_pdf(source_file, target_file, word_app)
...
def handle_replace(args):
    batch_replace_files(args.rootdir, args.search, args.replace)


def handle_file_name_check(args):
    batch_file_name_check(args.rootdir, args.pattern)


def handle_file_integrity_check(args):
    batch_file_integrity_check(args.rootdir, args.input, args.algorithm)


def handle_file_permission_check(args):
    batch_file_permission_check(args.rootdir, args.mode)
...
def handle_hash(args):
    ...
    batch_hash(args.rootdir, hash_algorithm=args.algorithm,
...
def handle_convert(args):
    batch_convert_files(
        ...
        format=args.format.upper(), recursive=args.recursive, word_app=args.wordApp
    )
...
def main():
    ...
    replace_parser = subparsers.add_parser(
        'replace', help='Batch replace file content'
    )
    ...
    replace_parser.add_argument(
        '--search', type=str, required=True,
        help='Regular expression'
    )
    replace_parser.add_argument(
        '--replace', type=str, required=True,
        help='String'
    )

    # name-check options
    name_check_parser = subparsers.add_parser(
        'name-check', help='Batch file name check'
    )
    name_check_parser.add_argument(
        '-r', '--rootdir', type=str, required=True,
        help='Root directory'
    )
    name_check_parser.add_argument(
    ...
    integrity_check_parser = subparsers.add_parser(
        'integrity-check', help='Batch file integrity check'
    )
    ...
    permission_check_parser = subparsers.add_parser(
        'permission-check', help='Batch file permission check'
    )
    ...
    convert_parser.add_argument(
        '-wa', '--wordApp', default='MS',
        help='using Microsoft office or WPS office'
    )
    ...
    subcommand_handlers = {
        ...
        'name-check': handle_file_name_check,
        'integrity-check': handle_file_integrity_check,
        'permission-check': handle_file_permission_check,
